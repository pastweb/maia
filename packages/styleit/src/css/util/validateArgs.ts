...
export function validateArgs({ validate = {}, forward, fileName, name }: StyleOptions): void {
  ...
  if (toValidate.length && forward) {
    toValidate.forEach(([ argName, validFunc]: [string, (arg: any) => boolean]) => {
      if (forward[argName] && !validFunc(forward[argName])) {
        throw new Error(`@maia/styleit - css validation error in: ${fileName ? `${fileName} -` : ''}${name} for arg "${argName}"`);
      }
    });
  }
}
