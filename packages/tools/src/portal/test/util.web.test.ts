...
describe('portal - util', () => {
  ...
  describe('open', () => {
    ...
    it('firstAppId should be inside portals[portalId]', () => {
      expect(Object.keys(portals[portalId]).includes(firstAppId as string)).toBe(true);
    });

    it('secondAppId should be inside portals[portalId]', () => {
      expect(Object.keys(portals[portalId]).includes(secondAppId as string)).toBe(true);
    });

    it('portals[portalId][firstAppId] should contains the instance of FirstApp class', () => {
      expect(portals[portalId][firstAppId as string] instanceof FirstApp).toBe(true);
    });

    it('portals[portalId][secondAppId] should contains the instance of SecondApp class', () => {
      expect(portals[portalId][secondAppId as string] instanceof SecondApp).toBe(true);
    });

    it(`the element with id="${firstAppId}" should be present into the DOM`, () => {
      expect(typeof document.getElementById(firstAppId as string) !== null).toBe(true);
    });

    it(`the element with id="${secondAppId}" should be present into the DOM`, () => {
      expect(typeof document.getElementById(secondAppId as string) !== null).toBe(true);
    });
  ...
  describe('update', () => {
    it('the FirstApp should contains the "newFirstAppValue"', () => {
      update(portals, { portalId, appId: firstAppId as string, appData: 'newFirstAppValue' });
      const element = document.querySelector('.firstApp') as HTMLElement;
      expect(typeof element !== null).toBe(true);
